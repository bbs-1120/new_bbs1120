          ...
          <table className="w-full min-w-[700px]">
            ...
            <tbody className="divide-y divide-slate-200">
              {sortedResults.map((result, index) => {
                const isStop = result.judgment === "停止";
                return (
                <tr
                  key={result.cpnKey}
                  className={`hover:bg-slate-50 ${
                    selectedKeys.includes(result.cpnKey) ? "bg-indigo-50" : ""
                  } ${isStop ? "bg-slate-100 opacity-60" : ""}`}
                >
                  ...
                  <td className="px-2 lg:px-3 py-2 lg:py-3 min-w-[100px] lg:min-w-[180px]">
                    <div className="flex items-center gap-1">
                      <p className={`text-[10px] lg:text-sm font-medium break-all line-clamp-2 flex-1 ${isStop ? "text-slate-500" : "text-slate-900"}`} title={result.cpnName}>
                        {result.cpnName}
                      </p>
                      <button
                        onClick={() => { setSelectedCpnForDetail(result.cpnName); setShowCpnDetailModal(true); }}
